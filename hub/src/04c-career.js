...
const Career = {
  ...
  open() {
    ...
    const evCard = (e, k, boss) => { const id = boss ? 'boss' + ci : Career.evId(ci, k), won = boss ? c.boss[ch.boss] : c.won[id], lock = boss && !Career.unlockedBoss(ci) && !won, tr = TRACKS[e[1]], nm = boss ? T('duel') + ': ' + (ch.name[L] || ch.name.ru) : e[2][L] || e[2].ru;
